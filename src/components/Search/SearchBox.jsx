...
//         className="flex items-center bg-white rounded-md w-full border overflow-hidden focus-within:ring-2 focus-within:ring-[#35AFA0]"
//       >
//         <input
//           value={q}
//           onChange={(e) => {
...
export default function SearchBox({ className = "" }) {
  ...
  return (
    <div ref={wrapRef} className={`relative w-full ${className}`}>
      <form
        onSubmit={handleSubmit}
        className="flex items-center bg-white rounded-md border overflow-hidden focus-within:ring-2 focus-within:ring-[#35AFA0]"
      >
        <input
          value={q}
          onChange={(e) => {
          ...
          onFocus={() => setOpen(true)}
          placeholder="Search for products..."
          className="px-3 sm:px-5 py-3 text-xs sm:text-sm outline-none flex-1 focus:outline-none"
        />
        <button
          type="submit"
          className="px-3 sm:px-4 py-2 flex items-center justify-center text-xs sm:text-sm font-semibold"
        >
      ...
      {/* Dropdown */}
      {open && (loading || list.length > 0) && (
        <div className="absolute z-30 mt-2 w-full bg-white border rounded-xl shadow-lg overflow-hidden max-h-80 overflow-y-auto">
          {loading && (
            <div className="p-3 text-sm text-gray-500">Searching…</div>
          )}
          {!loading && list.length === 0 && (
